...
const PeerReviewForm: React.FC = () => {
    ...
    return (
        <div className="container mx-auto p-4 flex flex-col h-full">
            <h1 className="text-3xl font-bold mb-4">Peer Evaluation</h1>
                ...
                <AnimatePresence initial={false} custom={direction}>
                    ...
                    >
                         <div className="p-6 bg-white rounded-lg shadow-md">
                            {conflict ? (
                                <>
                                    <label htmlFor="reason" className="block text-lg font-semibold mb-3">Reason for conflict:</label>
                                    <textarea id="reason" value={reason} onChange={e => setReason(e.target.value)} className="w-full h-32 p-2 border rounded" required />
                                </>
                            ) : (
                                <>
                                       ...
                                       <div className="flex items-center space-x-4">
                                            {[1, 2, 3, 4, 5].map(score => (
                                                <label key={score} className="cursor-pointer">
                                                    <input type="radio" name={`score-${question.code}`} value={score} checked={answers[question.code]?.score === score} onChange={() => handleAnswerChange(question.code, score, 'rating')} className="sr-only" />
                                                    <span className={`w-10 h-10 rounded-full flex items-center justify-center border-2 ${answers[question.code]?.score === score ? 'bg-blue-500 text-white' : 'bg-gray-200'}`}>{score}</span>
                                                </label>
                                            ))}
                                        </div>
                                    ) : (
                                        <textarea className="p-2 border rounded w-full h-32" placeholder="Comments" value={answers[question.code]?.response || ''} onChange={e => handleAnswerChange(question.code, e.target.value, 'text')}></textarea>
                                    )}
                                </>
                            )}
                        </div>
